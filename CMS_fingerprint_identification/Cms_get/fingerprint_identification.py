...
class fingerprint_identification(object):
    def __init__(self,domain_list,headers_list,proxy_list,time_out,association_number):
        ...
        if ('None' in self.proxy_list) :
            self.handler.append(urllib.request.ProxyHandler(proxies = None))
        else :
            for value in self.proxy_list:
                self.handler.append(urllib.request.ProxyHandler(proxies = eval(value)))          
    ...
    def get_title_server_dev(self,url):
        ...
        opener  = urllib.request.build_opener(random.choice(self.handler)) #是否使用代理
